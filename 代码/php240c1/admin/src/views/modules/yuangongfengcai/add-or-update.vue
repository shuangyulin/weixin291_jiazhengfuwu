...
export default {
  ...
  methods: {
	...
	this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(reg,'../../../php240c1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuangongfengcai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuangongfengcaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
