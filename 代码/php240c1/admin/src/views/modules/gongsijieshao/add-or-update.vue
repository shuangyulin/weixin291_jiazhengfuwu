<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="注册金额" prop="zhucejine">
          <el-input v-model="ruleForm.zhucejine" 
              placeholder="注册金额" clearable  :readonly="ro.zhucejine"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="注册金额" prop="zhucejine">
              <el-input v-model="ruleForm.zhucejine" 
                placeholder="注册金额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhuceshijian = true;
          ...
          if(o=='zhucejine'){
            this.ruleForm.zhucejine = obj[o];
	    this.ro.zhucejine = true;
            continue;
          }
          if(o=='xiangqing'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../php240c1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gongsijieshao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gongsijieshaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
